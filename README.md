...
- **FM 增强(端侧,隐私安全)**:
  - *候选重排*:打字停顿 ~0.4s 后,端侧模型根据上文把最合适的候选提到第 2 位(标 Apple 标志,不顶掉首选、防突然换词打错)
...
- **控制中心**(液态玻璃,嵌在输入法 bundle 内):菜单栏输入菜单「设置…」打开——一键安装/更新、浏览 698 万词库(拼音搜索+权重)、查看/管理用户词权重(打分/词频乘数)、设置(模糊拼音/全角标点/FM 增强/词频学习/候选字号),改动即时生效
...
open build/AFM拼音.app # 未安装态打开会自动弹出设置中心(一键安装);已安装后经菜单栏输入菜单「设置…」打开
```
...
- 中文模式:打拼音 → 数字 `1-8` 选词 / `空格` 上屏高亮候选 / `回车` 上屏拼音原文(网格展开时回车=空格上屏选中);选中候选为透明液态玻璃水滴,**按住可左右拖动,松手吸附最近候选直接上屏**
- `←→` 移动高亮(越过第 8 个自动展开网格),`↓` 展开候选网格(8 列,滚轮/滚动条滚动,边缘跟随,移回首行或顶行 `↑` 收起),`=`/`-` 翻页,候选条 `▾`/`▴` 点击展开收起,`Esc` 取消组词;网格内编号为水滴所在行的 1-8,数字键按行选词
...
├── AFMApp/         # 设置中心(液态玻璃;打包为引擎 bundle 内 PlugIns/AFMSettings.app,输入法菜单「设置…」打开)
