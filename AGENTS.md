...
- **候选窗交互定稿(2026-09-06,滑动窗口/网格/内联翻译)**:① **候选条 = 8 个滑动窗口(队列式)**:←→ 单步移动,选中越过边缘窗口跟移(barWindowStart),SwiftUI ForEach id diff + 按滑动方向的 asymmetric move/opacity transition 做队列动画(队首滑出/队尾滑入),取代 ◂▸ 翻页;数字键 1-8 = 窗口内位次(barWindowStart+d−1,数字 9 越界拒选),=/- 整窗翻页保留;② **↓ 展开 8 列网格**(gridColumns=8,可见 4 行):↑↓←→ 格间移动(±1/±8)行窗口上下滑动;从后面的行移回首行(候选 1-8)或顶行按 ↑ → 自动收起回候选条;候选条尾 ▾ / 网格底 ▴ 鼠标点击展开收起;候选查询上限 30→100(maxCandidates)撑起滚动;③ **⌃F 内联翻译**:组词中把当前高亮候选的译文单行显示在候选框(spinner→译文),空格上屏译文,⌃F/Esc/←→/继续打字退出;**独立翻译框组件(TranslatePanelController)暂不启用**——代码保留不接线,⌃F 非组词时放行原生;④ **CLT 坑:本 SDK 的 @State 已是宏,CLT 环境找不到 SwiftUIMacros 插件,编译直接失败**——候选窗视图禁用 @State/@StateObject 等宏包装器,窗口状态全部放 InputController 以 props 传入(@FocusState/@Published/@ObservedObject/@Namespace 这类非宏包装器可用);⑤ 窗口状态全在 InputController(barWindowStart/gridRowStart/滑动方向),视图纯 props 渲染;⑥ **选中态 = 可拖拽的透明液态玻璃水滴(2026-09-06 kb7 起两轮迭代,学自 Kyant0/AndroidLiquidGlass LiquidBottomTabs,vendor/AndroidLiquidGlass 有源码)**:水滴透明质感 = **双层内容 trick**——幽灵行快照(强调色:数字 cyan/词加粗,与正常行逐像素同布局,ImageRenderer 离屏渲染,内容变化才重拍)经 **Core Image 折射 kernel** 处理后叠在玻璃胶囊上,水滴划过哪里哪里的内容被放大折射(等价安卓 combined backdrop+lens);折射数学 1:1 移植其 AGSL:胶囊 `sdRoundedRect` SDF + `circleMap(1−√(1−x²))` 边缘折射(SDF 梯度方向,amount 传负=向内拉)+ 七采样色散,参数 `lens(10px*press, −14px*press)`(静止不折射,内容直接透过玻璃)。**shader 交付路径(2026-09-06 kb9 定稿,用户装 Xcode 后)**:`Metal/DropletLens.metal`(SwiftUI layerEffect 函数,`#include <SwiftUI/SwiftUI_Metal.h>`,签名 `afmLensMask(position, SwiftUI::Layer, rect, refr, layerSize)`,**SwiftUI::Layer 无 extent()**,图层尺寸自己传参;sample/position 都是 user-space 点坐标)→ package.sh 用 `DEVELOPER_DIR=<Xcode> xcrun -sdk macosx metal -c + metallib` 编成 `default.metallib` 进 bundle → 运行时 `ShaderLibrary.afmLensMask(...)` + 幽灵行视图 `.layerEffect(shader, maxSampleOffset)`——**无 deprecated API、无快照、全 GPU**;无 Xcode 构建时 metallib 缺失 → 水滴退化为纯玻璃无折射(优雅降级)。此前两条死路(实测):老 CIKL `CIKernel(source:)` 可用但 deprecated;现代 MSL CI kernel 需要 `-fcikernel` 编译标记 + metallib Data,运行时编译器给不了且 MTLLibrary 无序列化 API。拖拽交互(**水滴不动 bar 动**,2026-09-06 kb11 定稿——用户明确:按住水滴拖的是整条 bar,水滴钉在原地):整条 DragGesture(minimumDistance 4,不与点选冲突),按住即抓起(水滴钉在按压处候选,frame 钉死);拖拽时 dragFraction(锚点)= 抓取位 − dx/格宽(左拖 → bar 左滑 → 更高候选经过水滴),行内容以 `.offset(rowOffset)` 从水滴下滑过,窗口起点随锚点自调(dragWindowStart,锚点保持在窗口槽位 2-5)且**行偏移对窗口跳变做补偿**(rowOffset = dx − (ws−wsBase)×格宽),拖拽中行动画置 nil(rootView 重建会中断进行中的手势,DragGesture 无 onCancel);**折射层与 bar 同步滑动靠 shader 采样位移**——contentShift(=−rowOffset)传进 afmLensMask,所有 layer.sample 加 shift,水滴矩形固定不动(比视图侧 offset 叠加 layerEffect 的坐标系歧义可靠);松手 onDrop 回调 → InputController 吸附取整 commitCandidate(轻点 = 位移 0 的拖拽,与点选语义统一)。**水滴胀出条外**(Kyant0 pressedScale 78/56≈1.39,水滴比 bar 高):面板重构为 容器(透明,含边距 10×12) ├ NSGlassEffectView(玻璃条=候选条矩形,onFrameChange 回报此矩形给伴随面板) ├ 水滴覆盖宿主(PassthroughHostingView hitTest 全 nil,事件归下层候选条;内容 = 玻璃胶囊 26+ glassEffect/<26 白色半透明 → 幽灵折射层 → 容器色/上缘高光);水滴静止高 0.875×条高,按压 1.35×+速度挤压拉伸,胀出条外;**坐标基准坑**:coordinateSpace("candBar") 必须挂在**带 padding 的条根**上(挂行上会差出 padding 位移);连续拖拽状态放 CandidateDropletModel 单例(@Published 非宏,视图直写局部刷新);网格态不拖拽,选中格保留 glassEffectID 流动
- **伴随面板(2026-09-05,⌃V 剪贴板 / ⌃F 翻译;同日三修——免激活/并存/直录)**:CompanionPanels.swift 单文件;① **定位**=候选条 frame(候选窗 onFrameChange 回调回写 latestCandidateFrame)下方 8px,剪贴板贴候选条**左缘**、翻译贴**右缘**(并存重叠时翻译挪剪贴板右侧,再放不下→剪贴板下方),下方放不下→上方,再不行钳回屏内;无候选条→光标所在屏幕右上角(visibleFrame 排除菜单栏/Dock);**两面板可并存,不互斥**;② **焦点模型是关键坑(三修核心)**:剪贴板面板**免激活**——NonKeyPanel(canBecomeKey=false) 纯展示+点击,进程保持 .prohibited 不抢焦点,客户端组词/候选条/打字原样存活。此前两面板都临时切 `.accessory`+activate,客户端失焦触发 deactivateServer→commitComposition 把首选强制上屏(=「⌃V/⌃F 中断预测」根因,日志实锤 ⌃F 开面板瞬间 raw='awsl' 被上屏成'安慰失恋');免激活面板没有 resignKey 时机,点外部收起改用全局鼠标监听(NSEvent addGlobalMonitorForEvents,走已授权的输入监控 TCC,只观察不消费);键盘选词(1-9/↑↓/⏎/Esc)路由进 InputController.handle(剪贴板可见且非组词中),组词中按键归组词、面板用点击;③ **翻译面板必须抢键盘**(要打字):打开前 suspendCompositionForPanel 挂起组词——状态留内存不上屏,deactivateServer 据此跳过提交,关闭还焦点后 activateServer 原样恢复预编辑态+候选条(挂起时掐 fmGeneration 防在途 FM 回写重开候选窗);④ **面板激活后自有 app 的事件仍回流 IMK handle(实测)**——handle 顶部 anyKeyWindow 全放行翻译框才能收到原始按键,此前字母被拼音引擎吞掉(raw='aws'、候选窗盖在翻译框上)=「翻译框无法输入」根因;放行必须置于 lastActive 赋值**之前**,防自有 controller 劫持"插入到光标"的目标;翻译框字母直录,中文 ⌘V 粘贴;⑤ **插入时序**:客户端持焦点(剪贴板路径)立即 flush(首选)+insertText;翻译面板持键则先 close 还焦点 → 0.15s 后插入;组词中先按空格语义上屏首选再插入;⑥ 剪贴板 ClipboardMonitor 常驻轮询 NSPasteboard changeCount(1s),只收文本(2 万字截断/50 条/去重置顶),落盘 Application Support/AFM拼音/clipboard-history.json;⑦ 翻译用 FMReranker.translate(方向自动:含中文→英,否则→中);⑧ 快捷键拦截规则:⌃V/⌃F 中英模式都拦,option/command/shift 组合不拦,自己的面板持键时 handle 全放行;⌘V 原生粘贴不受影响;已知取舍:终端里 ⌃V(quoted-insert)/⌃F(forward-char)被面板接管
