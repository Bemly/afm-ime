...
- **输入法设置嵌入系统设置(2026-09-06,Preferences.prefPane 机制,无现代 SDK 文档)**:系统设置 → 键盘 → 输入法详情页由 KeyboardSettings.appex(`com.apple.Keyboard-Settings.extension`,扩展点 com.apple.Settings.extension.ui)渲染,第三方输入法默认只有隐私警示文案;**选项区 = 它按约定路径加载输入法 bundle 内的 `Preferences.prefPane` 并实例化 NSPrincipalClass**(本机拆包实证:SCIM 自家面板在引擎 appex `Contents/Resources/Preferences.prefPane`,苹果第三方模板 PluginIM 在 app 顶层 `Contents/Resources/Preferences.prefPane`,其主 Info.plist 无任何指向 key → 纯路径约定无特判;KeyboardSettings 链接公开 PreferencePanes.framework,含 instantiatePrefPaneObject/prefPaneObject/loadOptionsForInputSourceID:/InputSourceOptionsView,entitlements 带 kCFPreferencesAnyApplication 偏好读写豁免 + storage.inputsources 豁免)。IMK 文档化入口只有 IMKStateSetting.showPreferences(输入法菜单项 → preferences.nib 或 Info.plist `InputMethodServerPreferencesWindowControllerClass` 独立窗口),未用。实现:`PrefsPane/AFMPrefsPane.swift` 单文件 `@objc(AFMPrefsPane) class AFMPrefsPane: NSPreferencePane`,loadMainView 纯代码搭 NSStackView 无 nib(返回前必须 layoutSubtreeIfNeeded——宿主可能直接取 frame);package.sh 用 `xcrun -sdk macosx swiftc -parse-as-library -emit-library -Xlinker -bundle` 编译(产物 MH_BUNDLE;CLT SDK 无 PreferencePanes 头,无 Xcode 构建自动跳过 pane,IME 本体不受影响;codesign 深签名自动覆盖 pane)。设置键 AFMEnglishMode/AFMFuzzyPinyin/AFMFullWidthPunct/AFMFMEnhance/AFMUserFreqEnabled 由 IMECore/UserPrefs.swift 现读现判(cfprefsd 跨进程失效保证外部改动即时生效),**键名与 pane rows 表逐字一致——两边都要改**;门控点:模糊音=引擎变体展开处、全角标点=handlePunctuation、FM=scheduleFMRerank + 无候选占位分支(否则关 FM 后"整句预测中"转圈不停)、词频=UserFreq hits/looseHits/boost。**性能教训**:门控读 UserDefaults 必须查询入口快照一次再下传——逐候选读实测热循环 0.20→0.37ms,快照后回 0.24ms。**测试载体教训**:dictbench 裸 CLI 进程的 UserDefaults.standard 是自己进程域,验证门控要 `defaults write dictbench ...` 而非写 IME 域(实测 模糊关 zongguo 只剩精确 宗国/总国,删键恢复 中国)。加载验证:proto/prefs_probe.swift 离线模拟 KeyboardSettings 路径(Bundle.load→NSClassFromString→initWithBundle→loadMainView)全过;系统设置详情页真机出现选项区待用户确认
- **部署铁律(2026-09-06)**:**日常重装禁止跑 scripts/install.sh**——它的 enable 流程先 disableAll 再 enable,而 TISEnableInputSource 对本包返回 noErr 但**活视图静默不收**,等效每次部署都把输入源摘掉(「改几次代码输入法就消失」的根因);日常部署只做 cp 换盘 + killall + open(首次收录后注册永久有效)。TIS 实时视图坏掉的症状:`TISSelectInputSource` 返回 -50(对未启用源 select 本身就是 paramErr,API 没坏);恢复法:打开 系统设置→键盘→输入法→编辑 面板触发同步,defaults 里的启用条目会被拉回活视图。注意 defaults 的 AppleEnabledInputSources 有条目 ≠ 活视图已启用(后者要登录扫描才重建);反复 register/enable 会产生灰色重复实例,注销重登收敛
