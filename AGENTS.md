...
- **候选窗交互定稿(2026-09-06,滑动窗口/网格/内联翻译)**:① **候选条 = 8 个滑动窗口(队列式)**:←→ 单步移动,选中越过边缘窗口跟移(barWindowStart),SwiftUI ForEach id diff + 按滑动方向的 asymmetric move/opacity transition 做队列动画(队首滑出/队尾滑入),取代 ◂▸ 翻页;数字键 1-8 = 窗口内位次(barWindowStart+d−1,数字 9 越界拒选),=/- 整窗翻页保留;② **↓ 展开 8 列网格**(gridColumns=8,可见 4 行):↑↓←→ 格间移动(±1/±8)行窗口上下滑动;从后面的行移回首行(候选 1-8)或顶行按 ↑ → 自动收起回候选条;候选条尾 ▾ / 网格底 ▴ 鼠标点击展开收起;候选查询上限 30→100(maxCandidates)撑起滚动;③ **⌃F 内联翻译**:组词中把当前高亮候选的译文单行显示在候选框(spinner→译文),空格上屏译文,⌃F/Esc/←→/继续打字退出;**独立翻译框组件(TranslatePanelController)暂不启用**——代码保留不接线,⌃F 非组词时放行原生;④ **CLT 坑:本 SDK 的 @State 已是宏,CLT 环境找不到 SwiftUIMacros 插件,编译直接失败**——候选窗视图禁用 @State/@StateObject 等宏包装器,窗口状态全部放 InputController 以 props 传入(@FocusState/@Published/@ObservedObject/@Namespace 这类非宏包装器可用);⑤ 窗口状态全在 InputController(barWindowStart/gridRowStart/滑动方向),视图纯 props 渲染;⑥ **选中态 = 可拖拽的透明液态玻璃水滴(2026-09-06 kb7 重做,学自 Kyant0/AndroidLiquidGlass 的 LiquidBottomTabs,vendor/AndroidLiquidGlass 有源码)**:安卓版的精髓 = 双层内容(正常层 + 强调色幽灵层透明叠放,水滴的 lens 镜头把幽灵层「照」出来,水滴划过哪里哪里内容变亮)+ 拖拽连续小数位置(DampedDragAnimation,松手 spring 吸附最近项)+ 按压缩放/速度挤压拉伸(squash & stretch)。macOS SwiftUI 无公开 lens shader,复刻方案:**幽灵层用胶囊 mask 露出**(与安卓 combined backdrop 等价),水滴本体 = glassEffect(.regular.interactive(), Capsule());拖拽 = 整条挂 DragGesture(minimumDistance 4,不与 onTapGesture 冲突),按住即抓起(水滴跳到按压处候选),拖动中 dragFraction 在相邻候选实际 frame 间线性插值(**候选宽度不一,不能用等宽 tabWidth 公式**,frames 由 cell 的 onGeometryChange(macOS 15+)回写 @Published frames 字典);**拖拽期间钳制在可见窗口内、绝不滑窗口——rootView 重建会中断进行中的手势**(DragGesture 无 onCancel,断掉后 dragFraction 永远悬挂);松手 onDrop 回调 → InputController 吸附取整 commitCandidate(轻点 = 位移 0 的拖拽,与点选语义统一);连续拖拽状态放 CandidateDropletModel(@Published 非宏,视图直写局部刷新,不经 props 每帧往返 InputController);<26 回退白色半透明胶囊;网格态不拖拽,选中格仍用 glassEffectID 流动变形
