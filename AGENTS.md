...
- **候选窗交互定稿(2026-09-06,滑动窗口/网格/内联翻译)**:① **候选条 = 9 个滑动窗口(队列式)**:←→ 单步移动,选中越过边缘窗口跟移(barWindowStart),SwiftUI ForEach id diff + 按滑动方向的 asymmetric move/opacity transition 做队列动画(队首滑出/队尾滑入),取代 ◂▸ 翻页;数字键 1-9 = 窗口内位次(barWindowStart+d−1);=/- 整窗翻页保留;② **↓ 展开 8 列网格**(gridColumns=8,可见 4 行):↑↓←→ 格间移动(±1/±8)行窗口上下滑动;从后面的行移回首行(候选 1-8)或顶行按 ↑ → 自动收起回候选条;候选条尾 ▾ / 网格底 ▴ 鼠标点击展开收起;候选查询上限 30→100(maxCandidates)撑起滚动;③ **⌃F 内联翻译**:组词中把当前高亮候选的译文单行显示在候选框(spinner→译文),空格上屏译文,⌃F/Esc/←→/继续打字退出;**独立翻译框组件(TranslatePanelController)暂不启用**——代码保留不接线,⌃F 非组词时放行原生;④ **CLT 坑:本 SDK 的 @State 已是宏,CLT 环境找不到 SwiftUIMacros 插件,编译直接失败**——候选窗视图禁用 @State/@StateObject 等宏包装器,窗口状态全部放 InputController 以 props 传入(@FocusState/@Published/@ObservedObject 这类非宏包装器可用);⑤ 窗口状态全在 InputController(barWindowStart/gridRowStart/滑动方向),视图纯 props 渲染
