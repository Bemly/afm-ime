...
│   └── 候选生成: 多路切分 + mmap 二分前缀查表 + 词格 DP 整句组词(单主路径 composeSentence)+ 用户词频乘法加成 + 内置用户词种子(kb40,version 门控补种/删除墓碑),按词频权重打分合并(见决策记录)
├── FM 层(大模型增强,kb36 起双通道:端侧 FM / 云端模型经 Provider 协议接入)
│   ├── 进程内 import FoundationModels(SystemLanguageModel),每次新建无状态 session
│   ├── 云端模型(kb36):FoundationModels 官方 Provider 协议(LanguageModel + LanguageModelExecutor,即 IMECore/CloudProvider.swift 的 CloudProviderModel/Executor,架构 = ProviderBench 验证版/WWDC26 Provider 最小实现)——上层仍是标准 LanguageModelSession,instructions→system/prompt→user 的映射由框架 transcript 完成,"以 fm 框架暴露的为准";线格式双选:openai(POST {base}/chat/completions,Bearer)与 anthropic(POST {base}/v1/messages,x-api-key + anthropic-version: 2023-06-01,max_tokens 1024 必带);FMReranker.respond 统一出口:云端启用且配置完整→云端(15s 超时),失败/未启用→回落端侧 FM,再失败→nil 静默纯词典;提示词四段(重排/整句/翻译中→英/翻译→中)可整段覆盖(ModelPrefs;设置中心草稿-应用模式,见设置中心条)
...
**AFM拼音.app(设置中心,2026-09-06;同日合并进引擎 bundle 为单一 App)**:设置中心是嵌在引擎 bundle **Contents/PlugIns/AFMSettings.app** 的独立进程 helper(id moe.bemly.AFMSettings)——引擎 LSUIElement 不能弹窗,helper 承载窗口;嵌套 bundle 不进 Launchpad/Spotlight 索引,顶层只有「AFM拼音」一个条目。七区:①安装:一键安装并启用/更新(运行于安装位置时无新包可换,提示重打包;部署铁律同款 rm+cp 换盘+killall+确认死透+open)/卸载/一键注销/直达输入源设置;②词库:dict.bin 存储序分页浏览 + 拼音前缀搜索(DictStore.records(from:limit:));③用户词:词/拼音键/次数/档内分/词频乘数,右键删除、清空、「导入用户词…」(kb40:JSON 同内置种子格式,合并=取较大次数/拼音仅缺省才补/解除墓碑);④翻译(kb34):端侧 FM 中↔英互译,TextEditor 输入 ⏎ 提交、方向自动(含中文→英)、结果可选中/复制;⑤模型(kb36):云端模型卡(启用开关/提供商预设 9 种含 OpenAI·Anthropic·DeepSeek·Kimi·智谱·通义·豆包·Ollama·自定义,选预设自动填地址格式模型名/接口格式双选 OpenAI 兼容 vs Anthropic/Base URL/API Key SecureField/模型名/测试连接=CloudProviderModel.probe 同款 Provider 协议最小请求)+ 提示词卡(kb36.1 草稿-应用模式:四段 TextEditor **预填当前生效值**(覆盖 ?? 内置默认),编辑不落盘,「应用」按钮才写入——与内置一致的段落清掉覆盖键以跟随内置更新,「全部恢复默认」=草稿填回内置并立即应用;键名引用 IMECore.ModelPrefs 常量,单一来源不再两边逐字维护);⑥设置:模糊拼音/全角标点/FM 增强/词频学习四门控 + 候选条字号(13-22)+ 快捷键卡(kb35:剪贴板/内联翻译/设置中心三槽,录制 ⌃+字母/数字/符号——NSEvent 本地监控吞键、冲突检查、Esc 取消、onDisappear 撤监控;keyCode+显示字符双键落盘 AFMHotkey*/*Display,IME 只读键码、object 判 nil 区分「未设置」与键码 0(A),现读现判即时生效;修饰键固定 ⌃);⑦测试(kb38):实验交互开关区——固定透镜水滴拖拽(UserPrefs 键 AFMDropletFixedLens,默认关)——写 IME 域 defaults(`UserDefaults(suiteName: moe.bemly.inputmethod.AfmIME)` = IME 进程的 .standard 域),IME 现读现判即时生效。**入口**:⌃S 快捷键(kb34,中英模式都拦;**⌃F 维持组词中内联翻译不动**——中间态曾把 ⌃F 改为开设置,用户否决,定稿 ⌃F=内联翻译/⌃S=设置)/输入法菜单「设置…」(InputController.menu→doCommandBySelector,AFM拼音激活时菜单栏可见)/未安装态直接打开 bundle 自动拉起(isBundleInstalled 守卫)。**定位父引擎**:helper bundle 上三级(PlugIns→Contents→引擎.app);**图标(kb37)**:单一来源 Data/logo.png(1254² 液态玻璃「拼」设计稿),package.sh 构建期派生双图标——AppIcon.icns(iconset 16/32/128/256+2x,引擎/helper 双包 CFBundleIconFile)与 icon.tiff(16pt 多表示=tiffutil 合 16px@72dpi+32px@144dpi 两页,32px 页必须标 144dpi 折算回 16pt,否则输入菜单选项行高撑成两倍;Info.plist tsInputMethodIconFileKey/tsInputMode*IconFileKey 四键引用=系统设置输入法列表+输入菜单图标);坑:sips 转 PNG 必须显式 -s format png,否则输出仍是 TIFF 内容 iconutil 拒收;AFMApp 运行时图标回退读 AppIcon.icns;旧程序化占位图标与 make_icon.swift 生成器已删。**幽灵副本事故(23:03,kb32→kb33)**:幽灵副本退出前的延迟窗口里用同一连接名初始化了完整引擎(IMKServer/剪贴板/ShiftTap 全启动),劫持了客户端 IMK 连接,退出后连接断掉——窗口期(22:50-23:06)开着的应用全部打不了字,只有重启后活跃打字的应用自动重连;修法=自愈逻辑前移到 IMKServer 初始化之前同步 lsregister+立即 exit(0),不碰任何引擎设施;**余波恢复**:受影响应用退出重开一次即重建连接,系统级 killall Finder / killall Dock 即刷,顽固时注销重登。**液态玻璃规范**:@main WindowGroup + NavigationSplitView(系统玻璃 chrome 自动生效),高频动作 .toolbar、搜索 .searchable,内容区仅 .glassEffect 卡片不铺不透明材质;视图宏安全(@State/@StateObject 禁用,ObservableObject 家族,CLT 回退可编译;main.swift 文件名不能承载 @main,入口文件为 AFMApp.swift)
...
scripts/make_pkg.sh     # 打 .pkg 安装器: build/AFM拼音-<版本>.pkg(payload→/Library/Input Methods,双击/launcher 安装)
...
- **内置用户词种子 + 导入 + 删除墓碑(2026-09-15 kb40)**:作者本机打字积累的用户词导出为 **Data/builtin-user-words.json**(3581 条/240K,词频+拼音键,随仓库分发=新装用户开箱即得作者词库;数据即作者个人打字习惯,仓库 public)——UserFreq init 读 bundle Resources 同名文件,**version 门控补种**:文件 version > defaults 标记 AFMBuiltinUserWordsSeeded 才合入,只补缺失词(已有词取文件与本地较大次数,拼音仅本地缺省才补),补种后写标记(文件缺失/损坏不写标记,下次启动重试);**删除墓碑 AFMUserWordsDeleted**——设置中心删除/清空(清空=全量记墓碑)后,种子版本升级也不复活删过的词,导入显式带回=解除墓碑;**跨进程一致性(顺带修的老隐患)**:设置中心进程直写 IME 域 defaults(导入/删除/清空),而引擎持有内存副本+2s 防抖保存——不广播的话删掉的词会被引擎下次保存复活,故发 Darwin 通知广播(CFNotificationCenterGetDarwinNotifyCenter;libnotify 的 notify_* 在 Swift 无模块可 import),引擎收到后丢弃防抖窗口内未落盘改动并重读 defaults(外部改动优先);JSON 双 bundle 都拷(package.sh:引擎+helper Resources),谁先启动都完成补种;设置中心「导入用户词…」=NSOpenPanel 收同格式 JSON,合并规则与补种一致。佐证脚本 proto/seed_check.swift(13 case:补缺/不覆盖本机/幂等/墓碑挡版本升级/导入解墓碑)
...
- **模型页与云端接入(2026-09-08 kb36)**:设置中心「模型」页 = 云端配置 + FM 提示词编辑;云端走 **FoundationModels 官方 Provider 协议**而非手搓 HTTP——CloudProviderModel(LanguageModel 描述符,capabilities 全声明与 ProviderBench 实测一致)+ CloudProviderExecutor(actor,transcript→线格式转换,一次性 appendText 回灌),上层 FMReranker.respond 统一出口(云端→回落端侧→nil),重排/整句/翻译三入口共用;**选此架构的理由**:提示词/请求结构完全沿用 fm 框架暴露面(instructions+prompt→transcript),换后端零改上层,ProviderBench/fm serve 已实测协议互通;defaults 键全部收敛进 IMECore/ModelPrefs.swift 常量,AFMApp 引用常量(修掉 UserPrefs 时代"两边逐字维护"的坑);API Key 明文存 IME 域(个人机可接受,UI 有提示);远程 http 会被 ATS 拦(loopback 豁免,Ollama 本机可用);translator/rerank 的解析(序号数字/looksLikeChinese)对云端输出同样生效,不合规输出自动回退词典
- **应用图标与输入法图标换设计稿(2026-09-15 kb37)**:设计稿为 ChatGPT 生成液态玻璃「拼」字图(用户指定),入库 Data/logo.png 作**唯一源**,多尺寸一律构建期从它派生,不手工维护;TIFF 多表示(1x+2x)用 `tiffutil -cat` 合页——sips 无多页合成能力,且 2x 页必须 `sips -s dpiHeight/dpiWidth 144` 把 32px 折算回 16pt 表示,TIS/菜单按 point size 取表示,dpi 不标会把输入菜单选项行高撑成两倍(make_icon.swift 时代已知约束,该占位图标生成器连同 Data/appicon.tiff、Data/icon.tiff 已删);iconset 补 256/512 档让 Finder 大图与「显示简介」清晰。换图后生效路径:输入菜单 `killall TextInputMenuAgent` 即刷;系统设置输入法列表图标重开设置面板生效(该列表缓存偏顽固时注销重登收敛);Finder/Launchpad 图标随 rm+cp 换盘自然更新
- **支线交互以测试开关落地(2026-09-15 kb38)**:droplet-relative-motion 支线(kb12 定稿的「水滴固定透镜,bar 刚体滑过」拖拽)基于旧滑动窗口实现,与 kb13 定稿候选条(恒显前 8、无窗口滑移)无法直接 merge——**语义移植**到现行为并以设置中心「测试」tab 的开关门控(UserPrefs.dropletFixedLens,默认关=kb11 水滴滑动语义),开关只决定下一次拖拽走哪套模型(beginDrag 快照);窗口机制以**拖拽期局部窗口**形式存在(dragWindowStart 仅 fixedLensDragging 时驱动 barView/幽灵行渲染,松手归零),hop 词宽补偿用抓取帧快照;支线经 `git merge -s ours` 空合并关闭(内容已移植,分支标签删除)。方法论沉淀:老支线撞上大改后的主干,不硬 merge,抽交互语义重接线+门控共存,既保既有定稿又给实验效果留试用入口
- **pkg 安装器与系统级安装位(2026-09-15 kb39)**:scripts/make_pkg.sh = pkgbuild(payload=引擎 app→`/Library/Input Methods`)+ postinstall(结束旧引擎;`launchctl asuser <uid> sudo -u <console用户>` 跑 TIS 操作——TIS 缓存/偏好全 per-user,root 直调会写进 root 域;--register-input-source 内部查重 + **--ensure-enabled 幂等启用**:活视图已启用直接跳过,enable() 的 disableAll 会摘掉在用输入源,升级路径绝不能走)+ productbuild。**自签应用证书签不了 pkg**:productbuild/productsign 校验「installer 签名身份」拒绝 EKU=codeSigning 的应用证书,codesign 直接签 flat pkg 是假成功(退出 0 但 pkgutil 仍报 no signature)——输出未签名 pkg,本机 `sudo installer -pkg … -target /` 可装;对外分发需 Developer ID Installer + 公证。**引擎配套**:IMEInstaller 增 systemIMEURL/installedURLs(),isBundleInstalled 双安装位(否则 /Library 安装时幽灵自愈误杀、设置中心误判未安装反复拉起),AFMApp 安装/更新在系统级位改提示走 pkg(防双份 bundle 同 id)。**工具链事件(重大,2026-09-10 系统更新后)**:Xcode-beta.app 被 26.6(Swift 6.3.3)顶替期间,新 SDK 的 FoundationModels interface 把 Provider API 藏进 `#if compiler(>=5.3) && $AsyncExecutionBehaviorAttributes` 特性门,6.3.3 编不过(全量重编报 `cannot find type LanguageModel`,CloudProvider/ProviderBench 全挂);**增量构建复用旧缓存,故障被掩盖一周**——任何 clean build 才会暴露;修法=package.sh 探测顺序改 Xcode-beta.app(27,Swift 6.4)优先,按次 DEVELOPER_DIR 不动全局 xcode-select;-Xswiftc enable-experimental/upcoming-feature 两种拼法均无效。教训:**改了 Swift 源以外的东西(升级 SDK/工具链)后必须 clean 全量构建一次再谈可用**
