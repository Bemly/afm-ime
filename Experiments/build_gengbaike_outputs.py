...
# -*- coding: utf-8 -*-
"""
合并浏览器同源拦截得到的分页备份(.bk_caps_*.json)，生成梗百科(UID 1544008396)
全部投稿视频标题的 JSON / CSV / Markdown 三份交付文件。

数据来源：浏览器空间页 https://space.bilibili.com/1544008396/upload/video
页面自身对 https://api.bilibili.com/x/space/wbi/arc/search 的 fetch 响应（ps=40，共 25 页）。
...
def load_pages():
    pages = {}
    for fp in sorted(glob.glob(os.path.join(HERE, ".bk_caps_*.json"))):
        with open(fp, encoding="utf-8") as f:
            obj = json.load(f)
        for k, v in obj.items():
            if not str(k).isdigit():  # 跳过 dom1 等非分页键
                continue
            pages[int(k)] = v  # 后加载的同页覆盖（内容一致）
    return pages
...
def main():
    ...
    # 去重（以 bvid 为准，保留首次出现）+ 按 created 倒序
    seen = set(); dedup = []
    for v in rows:
        if v["bvid"] in seen:
            continue
    ...
    meta = {
        "mid": MID, "name": NAME,
        "source": "https://space.bilibili.com/1544008396/upload/video (wbi/arc/search, ps=40 x25 pages)",
    ...
    # 1) JSON
    jpath = os.path.join(HERE, "gengbaike-videos.json")
    with open(jpath, "w", encoding="utf-8") as f:
        json.dump(meta, f, ensure_ascii=False, indent=2)

    # 2) CSV（Excel 友好 utf-8-sig）
    cpath = os.path.join(HERE, "gengbaike-videos.csv")
    ...
    # 3) Markdown
    mpath = os.path.join(HERE, "梗百科视频标题全集.md")
    with open(mpath, "w", encoding="utf-8") as f:
        f.write(f"# 梗百科（B 站 UID {MID}）全部投稿视频标题全集\n\n")
        ...
        f.write(f"- 采集方式：浏览器同源读取空间页 wbi/arc/search 接口，ps=40 共 25 页；已校验 25 页齐全、bvid 零重复、时间倒序无越序\n\n")
        f.write("| # | 日期 | 时长 | 标题 | BV |\n")
        f.write("|---:|---|---|---|---|\n")
        for v in videos:
